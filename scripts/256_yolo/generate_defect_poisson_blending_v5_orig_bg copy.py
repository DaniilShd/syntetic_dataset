...
def apply_adaptive_blend(defect: np.ndarray, background: np.ndarray,
                         component_mask: np.ndarray) -> np.ndarray:
    """
    ★ Плотное смешивание без полупрозрачности
    """
    h, w = component_mask.shape
    mask_float = component_mask.astype(np.float32)
    
    # Адаптируем яркость краёв
    defect = adapt_defect_brightness(defect, background, mask_float)
    
    # ★ УМЕНЬШЕНО размытие маски — меньше полупрозрачности
    mask_float = cv2.GaussianBlur(mask_float, (9, 9), 4)
    
    # Ядро дефекта — БЕЗ примеси фона
    ...
    # Основная маска
    main_mask = cv2.GaussianBlur(mask_float, (9, 9), 4)
    
    # Расширенная маска для перехода
    kernel_outer = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
    outer_mask = cv2.dilate(mask_float, kernel_outer)
    outer_mask = cv2.GaussianBlur(outer_mask, (21, 21), 10)
    
    core_mask_3ch = np.stack([core_mask] * 3, axis=-1)
    main_mask_3ch = np.stack([main_mask] * 3, axis=-1)
    outer_mask_3ch = np.stack([outer_mask] * 3, axis=-1)
    
    # ★ Ядро: 100% дефект (без примеси фона)
    result_core = defect * core_mask_3ch + background * (1 - core_mask_3ch)
    
    # Средний слой: 85% дефект
    blend_mid = defect * 0.85 + background * 0.15
    mid_weight = main_mask_3ch - core_mask_3ch
    mid_weight = np.clip(mid_weight, 0, 1)
    
    # Внешний слой: 50% дефект
    blend_outer = defect * 0.5 + background * 0.5
    outer_weight = outer_mask_3ch - main_mask_3ch
    outer_weight = np.clip(outer_weight, 0, 1)
...
def get_histogram_similarity(img1: np.ndarray, img2: np.ndarray) -> float:
    ...
    # V: яркость — 0.6 (увеличено, ГЛАВНЫЙ критерий)
    hist1 = cv2.calcHist([hsv1], [2], None, [32], [0, 256])
    hist2 = cv2.calcHist([hsv2], [2], None, [32], [0, 256])
    score += 0.6 * cv2.compareHist(hist1, hist2, cv2.HISTCMP_BHATTACHARYYA)
...
class PoissonDefectGenerator:
    ...
    def get_matching_background(self, target_img: np.ndarray, max_attempts: int = 200) -> np.ndarray:
    ...
    def process_image_all_defects(self, img_path: Path, all_bboxes: List[Dict],
        ...
        try:
            ...
            for bbox_idx, bbox_info in enumerate(all_bboxes):
                ...
                center = (float(crop_w) / 2, float(crop_h) / 2)
                rot_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)

                defect_rotated = cv2.warpAffine(defect_crop, rot_matrix, (crop_w, crop_h),
                                                flags=cv2.INTER_LINEAR,
                                                borderMode=cv2.BORDER_REFLECT)
                mask_rotated = cv2.warpAffine(crop_comp_mask.astype(np.float32), rot_matrix, 
                                            (crop_w, crop_h),
                                            flags=cv2.INTER_LINEAR)
                mask_rotated = (mask_rotated > 0.3).astype(np.uint8)

                if mask_rotated.sum() < 16:
                    continue

                                # ★ ОПРЕДЕЛЯЕМ БЛИЗОСТЬ К ГРАНИЦАМ (ДО поворота)
                at_left = x1 <= 0
                at_top = y1 <= 0
                at_right = x2 >= img_w
                at_bottom = y2 >= img_h
                near_edge = at_left or at_top or at_right or at_bottom
                
                # ★ ЕСЛИ У ГРАНИЦЫ — НЕ ПОВОРАЧИВАЕМ И НЕ СМЕЩАЕМ
                if near_edge:
                    defect_final = defect_crop
                    mask_final = crop_comp_mask
                    x1_new, y1_new = x1, y1
                    x2_new, y2_new = x2, y2
                else:
                    # ★ ПОВОРОТ
                    ...
                    # ★ СМЕЩЕНИЕ
                    max_shift = int(img_w * 0.3)
                    shift_x = random.randint(-max_shift, max_shift)
                    shift_y = random.randint(-max_shift, max_shift)
                    
                    x1_new = x1 + shift_x
                    y1_new = y1 + shift_y
                    x1_new = max(0, min(x1_new, img_w - crop_w))
                    y1_new = max(0, min(y1_new, img_h - crop_h))
                    x2_new = x1_new + crop_w
                    y2_new = y1_new + crop_h
                    
                    defect_final = defect_rotated
                    mask_final = mask_rotated
                
                # Сглаживание маски
                mask_blurred = cv2.GaussianBlur(mask_final.astype(np.float32), (5, 5), 2)
                mask_blurred = (mask_blurred > 0.3).astype(np.uint8)
                if mask_blurred.sum() == 0:
                    continue
                
                # SD генерация
                defect_crop_pil = Image.fromarray(defect_final)
                ...
                background_crop = background[y1_new:y2_new, x1_new:x2_new].copy()
                blended = apply_adaptive_blend(generated_np, background_crop.astype(np.float32), mask_blurred)
                
                if blended.shape[:2] != (crop_h, crop_w):
                    blended = cv2.resize(blended, (crop_w, crop_h))
                
                result[y1_new:y2_new, x1_new:x2_new] = blended.astype(np.float32)
                
                # ★ Аннотация с новыми координатами
                new_x_center = (x1_new + crop_w/2) / img_w
                new_y_center = (y1_new + crop_h/2) / img_h
                new_width = crop_w / img_w
                new_height = crop_h / img_h
...
def main():
    ...
    parser.add_argument("--sd_strength_max", type=float, default=0.12)
