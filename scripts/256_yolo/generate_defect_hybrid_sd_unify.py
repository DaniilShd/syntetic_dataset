...
Poisson Blending + SD для дефекта и фона + Масштабирование (больше!) + Color Correction
ИСПРАВЛЕНО: масштабирование YOLO-разметки, увеличенный диапазон scale_factors
"""
...
class PoissonBlendConfig:
    ...
    bg_color_correction_strength: float = 0.6

    def __post_init__(self):
        if self.scale_factors is None:
            # Только увеличение (без уменьшения) + анизотропное растягивание
            self.scale_factors = [
                (1.0, 1.0),    # оригинал
                (1.05, 1.0),   # растянуть по X
                (1.0, 1.05),   # растянуть по Y
                (1.1, 1.0),    # сильнее по X
                (1.0, 1.1),    # сильнее по Y
                (1.15, 1.0),   # ещё сильнее по X
                (1.0, 1.15),   # ещё сильнее по Y
                (1.1, 1.05),   # комбо
                (1.05, 1.1),   # комбо
                (1.2, 1.0),    # максимальное по X
                (1.0, 1.2),    # максимальное по Y
                (1.15, 1.15),  # равномерное увеличение
                (1.3, 1.0),    # экстремальное по X
                (1.0, 1.3),    # экстремальное по Y
            ]
...
def scale_defect_and_mask(background_crop: np.ndarray, crop_comp_mask: np.ndarray,
    ...
    """
    Анизотропное масштабирование дефекта (раздельно по X и Y).
    scale_factor: tuple (scale_x, scale_y) — только >= 1.0
    """
...
class SDDefectGenerator:
    ...
    def generate_background(self, bg_crop: Image.Image, seed: int = None) -> np.ndarray:
        ...
        # Минимальный strength для стабильности
...
def main():
    parser = argparse.ArgumentParser(
        description="SD фон + SD дефект + Масштабирование (больше!) + Color Correction"
    )
    ...
    parser.add_argument("--sd_strength_min", type=float, default=0.25)
    parser.add_argument("--sd_strength_max", type=float, default=0.45)
    parser.add_argument("--bg_strength_min", type=float, default=0.25)
    parser.add_argument("--bg_strength_max", type=float, default=0.35)
    ...
    parser.add_argument("--bg_sd_steps", type=int, default=20,
                   help="Количество шагов для SD-генерации фона")
    parser.add_argument("--bg_guidance_scale", type=float, default=1.5,
                    help="CFG scale для фона")
    ...
    config = PoissonBlendConfig(
        ...
        bg_sd_steps=args.bg_sd_steps,           # ← добавь
        bg_sd_guidance_scale=args.bg_guidance_scale,  # ← добавь
